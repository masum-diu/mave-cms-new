...
const CardsHeader = ({
  ...
  onSearch,
  uniqueTags, selectedTag, handleTagFilterChange,
  viewType, setViewType,
  cardsData = [],
}) => {
  const total  = cardsData.length;
  const active = cardsData.filter(c => c.status === 1).length;
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: "0.72rem", color: "#9ca3af" }}>
                Manage reusable card components
              </p>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <Stat label="Draft"  value={draft}  color="#f59e0b" />
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Tooltip title="Copy API endpoint">
            <Button
              icon={<CopyOutlined />}
              onClick={() => {
                navigator.clipboard.writeText(`${process.env.NEXT_PUBLIC_API_BASE_URL}/cards`);
